...
content and per-file line endings are preserved exactly. Idempotent: a
page that already has the widget id is skipped, so this can be re-run
after new pages are added.

NOTE: build_services.py's PAGE_TEMPLATE carries the same snippet, so
regenerated service pages keep the widget. Keep the two in sync.

Usage: python chat-widget.py
"""
import pathlib

ROOT = pathlib.Path(__file__).resolve().parent
WIDGET_ID = b"6a3602e7d896e3ca46331853"
SNIPPET = (
